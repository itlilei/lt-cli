...
; (function(global, factory) {
    ...
    let ua = navigator.userAgent.toLowerCase(),
        ...
        // 新增
        isMobilePhoneJdAppGte702 = isMobilePhone && isJdApp && jdAppVersionDigits >= 702,
    ...
    // ok
    function androidJsBridgeInitShare(paramsObj, over702) {
        let type = over702 || false;
        let temp = {
            'title': paramsObj.title,
            'content': paramsObj.content,
        ...
        if (type) {
            if (paramsObj.mpId && paramsObj.mpPath && paramsObj.mpIconUrl) {
                temp.mpId = paramsObj.mpId;
                temp.mpPath = paramsObj.mpPath;
                temp.mpIconUrl = paramsObj.mpIconUrl;
                temp.mpType = paramsObj.mpType;
            }
        }
        shareHelper.initShare(JSON.stringify(temp));
    }
    ...
    // ios高于v5.0.0版本的京东 & ios的非京东(ok)
    function iphoneIpadOpenAppGte500(paramsObj, over702) {
        let type = over702 || false;
        let jsonObj = {
            category: 'jump',
            des: 'share',
        ...
        if (type) {
            if (paramsObj.mpId && paramsObj.mpPath && paramsObj.mpIconUrl) {
                jsonObj.mpId = paramsObj.mpId;
                jsonObj.mpPath = paramsObj.mpPath;
                jsonObj.mpIconUrl = paramsObj.mpIconUrl;
                jsonObj.mpType = paramsObj.mpType;
            }
        }
    ...
    function processingChannel(inputChannels) {
        let androidShareChannel2IPhoneMap = {
        ...
        if (isInputParamObjLegal(inputParamObj)) {
            try {
                let targetParamObj = {
                        ...
                        mpIconUrl: null,
                        mpType: null
                    }, hasCallbackFunc, callbackSwitcher, hasClickCallbackFunc, clickcallbackSwitcher;
    ...
    /**
     *作用：下发分享设置（ver1.5.0beta）
     *版本：
     * jdApp7.0.2以上版本 额外 支持小程序分享设置
     * jdApp4.4.0以上版本支持基本分享设置(即使设置了小程序的分享也不生效不会报错)
     * 低于 jdApp4.4.0的部分版本可能支持(具体不详，但 2015年12月25日，Android 平台 V＜4.1，IPhone 平台 V＜4.3 已经强制升级)
    ...
    JdShare.prototype.setShareInfo = function(params) {
        try {
            let paramsObj = null, link = '', jsonObj = null;
            // 仅在 jdApp 内执行
            if (isJdApp) {
                // 保存各种配置信息
                ...
                if (isAndroid) {
                    ...
                    if (window.shareHelper) {
                        ...
                        if (typeof shareHelper.initShare === 'function') {
                            ...
                            // *jdApp7.0.2版本新增分享小程序设置
                            androidJsBridgeInitShare(paramsObj, isMobilePhoneJdAppGte702);
                    ...
                    if ((isMobilePhoneJdAppGte500) || (isIpadJdAppGte360)) {
                        location.href = iphoneIpadOpenAppGte500(paramsObj, isMobilePhoneJdAppGte702);
     ...
     * 作用：唤起分享面板
     版本：
     * jdApp 7.0.2 支持小程序分享设置，会占用普通微信分享
     * jdApp 5.0 支持唤起自定义面板
    ...
    JdShare.prototype.callSharePane = function(params) {
        try {
            let link, jsonObj = null, paramsObj = null, shareParam;
            // 保存各种配置信息
            paramsObj = processingInputParam(params);
